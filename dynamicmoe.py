...
class MLPDynamic(nn.Module):
    def __init__(self, input_size=768, hidden_size=256, output_size=2, projected_output_size=2):
        ...
        # self.fc5 = nn.Linear(in_features=1000, out_features=output_size)        
        # self.mapper = nn.Linear(in_features=output_size, out_features=projected_output_size)
        # self.relu = nn.ReLU()
...
class DynamycMoE(nn.Module):
    ...
    def __init__(self, input_size=768, hidden_size=1000, noisy_gating=True, k=4, kind="dynamic", total_cls=100):
        super().__init__()
    ...
    def _prob_in_top_k(self, clean_values, noisy_values, noise_stddev, noisy_top_values):
        ...
        # print(f"(clean_values - threshold_if_in)/noise_stddev: {(clean_values - threshold_if_in)/noise_stddev}")
    ...
    def forward(self, x, labels, is_training=True, loss_coef=1e-2):
        """Args:
        ...
        gates, load = self.noisy_top_k_gating(x, is_training)
...
if __name__ == "__main__":
    ...
    # e1 = 20 units -> 20 old + 20 new with 0 weights -> 20 + 20 (0) + 20 (0)
    # e2 = 20 units -> 20 new with 0 weights + 20 new -> 20 (0) + 20 + 20 (0)
    # e3 = 20 units -> 20 (0) + 20 (0) + 20

    # e1 = 20 units -> 20 old + 20 new not 0 + 20 new with 0 weights 
    # e2 = 20 units -> 20 new with 0 weights + 20 new with 0 weights + 20 new
